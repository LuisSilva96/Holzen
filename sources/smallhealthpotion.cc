...
constexpr unsigned HEALTH = 5;

SmallHealthPotion::SmallHealthPotion() :
  cure(HEALTH)
{
  QPixmap sprite(":/objects/sprites/smallhealth.png");

  set_object_sprite(sprite);
}

SmallHealthPotion::SmallHealthPotion(const QVector2D & _position)
  : cure(HEALTH)
{
  QPixmap sprite(":/objects/sprites/smallhealth.png");
...
void SmallHealthPotion::behave(Entity & hero)
{
  dynamic_cast<Hero &>(hero).increment_life(this->cure);
}

Object_Type SmallHealthPotion::get_type()
{
  return Object_Type::SMALLHEALTHPOTION;
}
